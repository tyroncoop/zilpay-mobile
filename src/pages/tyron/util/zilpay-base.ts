...
export class ZilPayBase {
  ...
  async deployDid(
    ...
    try {
      ...
      const getCode = await zilliqa.blockchain.getSmartContractCode(XWALLET);
      const code: any = getCode.result?.code;
  ...
  async deployDomain(
    ...
    try {
      ...
      const getCode = await zilliqa.blockchain.getSmartContractCode(addr);
      const code: any = getCode.result?.code;
      ...
      const MSG_VERSION = 1;
      const VERSION = bytes.pack(CHAIN_ID, MSG_VERSION);
      const gasLimit = Long.fromNumber(35000);
      const gasPrice = units.toQa("2000", units.Units.Li);
      const contract = contracts.new(code, init);
      const [tx, deployed_contract] = await contract.deploy({
        version: VERSION,
  ...
  async deployDomainBeta(net: string, username: string, privkey: string) {
    try {
      //@xalkan
